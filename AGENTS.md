...
- **Current version**: v5.17.1 (Fix: forced transfers now catch players absent from allXP — `computeAllXP` excludes status !== "a"/"d", so injured/left-league players like Watkins (status "u") and Mateta (status "i") were invisible to the optimizer; Phase 0 now builds a bootstrap stub for missing players and forces their replacement. Warning shows "OUT → IN" format.)
