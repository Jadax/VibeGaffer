...
- **v5.12.0 shipped** — "one-stop-shop" feature drop, scoped against LiveFPL/FPL Review/FFix/fpl.team patterns:
  ...
  3. **Predicted Lineups** (Fixtures tab) — `VG.predictedLineups(gw, fixtures)`: per-team projected XI + bench from the same `computeFixtureXP().xMins` signal the xP engine trusts, default 4-3-3 with surplus-only promote/demote to hit formation minimums (GK 1 / DEF 4 / MID 3 / FWD 1) without ever breaking one minimum to fix another. Gated on `VG.recentFormMaxRounds >= 2`; pre-season renders an unlock notice. **Bug found in browser smoke**: the GK was never added to `xi` (all 20 teams projected 10 men), and the original promote pass could drop DEF below 4 to add a FWD — both fixed; regression tests assert exact 11-man XIs with legal minimums and that bench never duplicates the XI.
  4. **Clean-Sheet & xGC Outlook** (Fixtures tab) — `VG.teamDefensiveOutlook(gw, fixtures)`: per-team next-fixture `xgf`/`xgc` from the same 1000-scale Elo/strength numbers the engine uses (`xgf = 1.35 + (att − oppDef)/300 ± 0.22 home`), `P(CS) = e^(−xgc)`, sorted by CS odds descending with ranks. Display-only model; safe to recompute because `computeTeamElo` is idempotent (seeds from the `_eloBase` snapshot `buildMaps` takes).
  5. **Form vs Fixture Difficulty scatter** (Compare tab) — `VG.formFixturesData(allXP, fixtures, gw)` (pure, testable) + `VG.render.formFixturesChart(canvasId, allXP, fixtures, gw)` (thin Chart.js 4.4.0 wrapper, `typeof Chart === "undefined"` guard, `canvas._chart` destroy pattern). X = next-GW FDR (blank → 5), Y = form, coloured by position, whole pool.
  6. **Elo idempotency fix (v5.10 bug, real)**: `computeTeamElo` read live `strength_*` fields that it itself mutates, so repeated calls compounded drift. `buildMaps` now snapshots `t._eloBase` (post-fallback strength + rating); `computeTeamElo` seeds from `_eloBase`. Regression test asserts defensive-outlook recomputation is byte-identical.
  - Version sync: package.json + all four `docs/index.html` cache-busters now `5.12.0` (the `ui.js?v=` bump was the one that got missed and tripped the CI release-assets check).
  - Tests: 216 → 246 (30 new: rollValue paths + null-safety, briefing structure/captain/transfer-source/null, lineups pre-season notice + 11-man XIs + minimums + formation-label + bench-disjoint + escaping, CS outlook coverage/sort/ranks/idempotency/render, scatter datasets/FDR bounds). Test count in the "all ten public tabs" check now includes `briefing`.
  - Verified live in-browser (python http.server + Playwright): Squad render, Briefing tab (outlook 2.82 FDR, captain + VC, roll-vs-spend transfer, price-risk chips, injury watch), Fixtures (unlock notice + 20-row sorted CS table, ARS 31.2% top), Compare scatter (4 datasets, 526 points), all ten tabs switch and preload. Only console output is the expected pre-season `bootstrap-lite.json`/`odds.json` 404s.
- **Previous**: v5.11.0 (architectural hardening and modular transport/UI layer on top of v5.10.0)
