...
- **Current version**: v5.15.0 (full-codebase correctness review)
- **v5.15.0 shipped** — review found and fixed real bugs; model output changes:
  1. **DEFCON double-count fix** — `xpDEFCON` was `defconXP * 2` after `defconXP` already contained the 2×P award; defenders/mids were inflated ~2 pts/fixture on DEFCON. Now face value.
  2. **Heavy-rotator lookup fix** — `congestionMultiplier` compared `String(numericTeamId)` against short-name codes and never matched; now resolves via `VG.teams[teamId].short_name`. The v5.14 congestion penalty actually fires now.
  3. **BGW detection fix** — blank teams are those ABSENT from a GW's fixture map (old filter looked for zero-valued entries and always returned `[]`). Planner, `teamSeasonRow`, chip calendar FH/BGW tiers now see real blanks.
  4. **Briefing chip hint fix** — chip identity comes from the object KEY (`evaluateChips` objects have no label field); hint now renders. Also iterates `Object.keys(ca)` instead of a hardcoded 4-chip list.
  5. **`allXP` mutation fix** — the Compare select sorted the shared xP-ordered array alphabetically in place, silently corrupting the Differentials quartile cutoff and the watchlist pool. Sorts a copy now.
  6. **Live double-count fix** — auto-subbed bench players were counted in BOTH `startPts` and `benchPts` (and rendered twice, with the subbed-out starter missing). Bench points now come only from `subResult.subs`; table = current XI + subbed-out starters + remaining bench.
  7. **`fetchTeamRank` fix** — `/entry/{id}/` has no `history` array; reads `summary_event_points`/`summary_overall_rank` (GW points were permanently 0).
  8. **Smaller fixes**: `_projGW` cleared after `computeMultiGWXP` (stale-gate leak); recency startsRate divides by games on record (not hardcoded 5); `goalsPer90`/`assistsPer90` clamped like their xG/xA siblings; missing-player fallback returns the full result shape; `oppWin` dedup; `predictBonus` guards `el.stats`; briefing prints `—` not `null` for blank avgFDR; ticker uses shared `VG.fdrColor`; TC non-DGW exception (85/82) can now actually recommend; transfer plan no longer skips near-zero-xP starters (they are the prime sell candidates); `_runRetries` resets on success; live refresh timer cleared on both branches; GW1 + team ID falls back to draft on 404 instead of forcing draft unconditionally; percentile helper hoisted to `VG._percentile` (was defined 3×); `mcGWDistribution` reuses `VG._mcDrawTotal`.
  - Tests: 286 → 300 (regression tests for every fix above + de-tautologized team-news/profile checks + synthetic congestion/BGW fixtures + test stubs now restore `VG.players` state).
- **Previous versions**: v5.14.0 pre-GW1 robustness (congestion, three-phase confidence, deadline countdown, VC discount, efficiency score, position home boost) · v5.13.0 transfer + foreign-signing awareness (`team_code` key, new-club multiplier, foreign priors) · v5.12.0 Briefing tab, lineups, CS outlook, scatter · v5.11.0 transport/UI module split · v5.10.0 Elo layer · v5.9.0 domain/design/copy pass · v5.8.0 recency windows, watchlist, What-If, Rate My Team · v5.7.0 race simulator · v5.6.0 profiles, rank, news, chip calendar · v5.5.0 EO, MC distribution, planner, set pieces · v5.4.0 Understat enrichment · v5.3.0 live GW + smart captaincy.
