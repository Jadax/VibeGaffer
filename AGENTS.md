...
- **Current version**: v5.17.0 (Forced transfers for injured/left-league players — Phase 0 in `optimizeTransfers` guarantees unavailable players are replaced even when no value upgrade clears the threshold; Triple Captain can never fire when the squad has dead assets (`topCaptainCandidates` now filters by `VG.isAvailable` + `evaluateChips` zeros TC score for broken squads); customizable transfer constraints sidebar (maxTransfers, maxPrice, minEO, avoidTeams with code-to-ID resolution); Free Hit + Wildcard chip generators with dedicated squad builders and amber banner; pitch inverted GK-top/FWD-bottom; Transfers + Chip Strategy pinned above Metrics)
